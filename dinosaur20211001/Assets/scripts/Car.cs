using UnityEngine;     //引用unity 引擎命名空間:可以使用此空間的 API
...
public class Car : MonoBehaviour
{
    ...
    private void Start()
    {
        ...
        print("重量轉換:" + CovertWeight (weight));
    }
    #endregion

    #region 方法語法
    //方法:保存程式內容的區域、演算法、陳述式
    //語法:修飾詞 傳回類型 方法名稱(參數1、參數2...) { 程式內容 }
    //void 無傳回:使用方法時不會傳回資料
    //自訂方法:不會執行、必須呼叫才會執行
    private void Test()
    {
        //輸出(訊息)-將訊息輸出至Unity 儀表板Console
        print("哈囉，沃德:D");
